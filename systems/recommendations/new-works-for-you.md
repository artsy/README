...
"New Works For You" (NWFY)&mdash;and its close relative "Lots For You"&mdash;is the only recommendations surface with an omni-channel presence. Each channel implements a slightly different version, but all versions use the User Artist Affinity Model to score eligible artworks and recommendations using the Genomic-based Similarity Model as a backfill. Since the user artist affinity model produces a score for an artist, all their artworks inherit the same score, whereas the genomic model produces a score for the artwork.

The sequence diagram of NWFY is shown below.

```mermaid
sequenceDiagram
    participant C as Mobile/Web/Email Client
...
- $\lambda > 0$ controls the half-life (larger = slower decay).
...
- $\Delta t$ is the time elapsed in days since the action.
- $\lambda$ is a decay constant for all users (e.g., a 90-day old action is discounted the same for active and lapsed users).
...
1. _Eligible User_ := Active user in the last $M$ months (based on Thursday-Notification send log or platform sessions log).
2. _Unfollowed Artists_ := Artists unfollowed by users in the last $D_1$ days and not currently being followed by _Eligible User_.
3. _Eligible Artworks_ := Artworks available for sale, published in the last $D_2$ days, not collected by _Eligible User_, and not from _Unfollowed Artists_.
...
12. _Deduplicated List_ := Union all recommendations, deduplicated by keeping the artwork with the highest _Final P-level_.
13. _NWFY Recommendations_ := Top-$`N`$ entities, after sorting _Deduplicated List_ using an intra-artist ranking to promote (not guarantee) the interleaving of artworks from an artist with artworks from other artists (see example below).

Example of intra-artist ranking.
...
1. _Eligible User_ := Logged in user in the last $M_1$ months or user who received a Thursday-Notification email in the last $M_2$ months.
2. _Unfollowed Artists_ := Artists unfollowed in the last $D_1$ days and not currently followed by _Eligible User_.
3. _Eligible Artworks_ := Artworks available for sale, published in the last $D_2$ days, not from _Unfollowed Artists_, not already recommended to _Eligible User_.
...
7. _Deduplicated List_ := Union all recommendations, deduplicated by keeping the artwork with the highest _Base P-level_.
8. _NWFY Recommendations_ := Top-$`N`$ entities, after sorting _Deduplicated List_ based on _Base P-level_ with ties resolved using affinity score.
