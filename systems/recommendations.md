...
Algorithmic recommendations surfaces (henceforth, just recommendations surfaces) are UI components featuring a list of entities (e.g., artworks, artists) whose selection and ranking are determined by algorithms that&mdash;among other subroutines like candidate generation and re-ranking&mdash;transform user signals (e.g., views, follows, saves, purchases etc) into scores&mdash;rather than _exclusively_ reflecting user inputs (e.g., filters, budget preferences) or contextual metadata (e.g., location). The scores produced by these algorithms typically represent the user's preference for each entity.
...
> ⓘ We'll introduce each scoring model in the context of the algorithm that uses it. We'll explain it once in the first algorithm that uses it and then reference it in the subsequent algorithms that use the same scoring model.
